...
def get_args() -> argparse.Namespace:
    ...
    parser.learning_algorithm = "gru_l_n_dgn_r"
    return parser
...
def watch(
    ...
    weights_path = os.path.join(args.logdir, "mpr", "gru_l_n_dgn_r", "weights", f"{args.model_name}")
...
def train_agent(
    ...
    if not args.optimize:
        # log
        log_path = os.path.join(args.logdir, 'mpr', 'gru_l_dgn')
        logger = CustomLogger(project='dancing_bees', name=args.model_name)
        writer = SummaryWriter(log_path)
        writer.add_text("args", str(args))
        logger.load(writer)
    weights_path = os.path.join(args.logdir, "mpr", "gru_l_n_dgn_r", "weights")
