...
from graph_env.env.utils.networks.gru_hl_dgn import RecurrentHLDGNNetwork
...
def get_args() -> argparse.Namespace:
    ...
    parser.learning_algorithm = "gru_hl_dgn"
    return parser
...
def watch(
    ...
    weights_path = os.path.join(args.logdir, "mpr", "gru_hl_dgn", "weights", f"{args.model_name}")
...
def train_agent(
    ...
    if not args.optimize:
        # log
        log_path = os.path.join(args.logdir, 'mpr', 'gru_hl_dgn')
        logger = CustomLogger(project='dancing_bees', name=args.model_name)
        writer = SummaryWriter(log_path)
        writer.add_text("args", str(args))
        logger.load(writer)
    weights_path = os.path.join(args.logdir, "mpr", "gru_hl_dgn", "weights")
